...
#######################################################################################################################
    ...
    async def cmd_yardım(self, command=None):
        """
        Usage:
            {command_prefix}yardım [command]
        ...
        if command:
            ...
            helpmsg += "```\n"
            helpmsg += "Türk Milleti Çalışkandır! | Ne Mutlu Türküm Diyene! Yapımcı: ℂ⋆EMIR KAYA#9899".format(self.config.command_prefix)

            return Response(helpmsg, reply=True, delete_after=60)

    async def cmd_davet(self, message, server_link=None):
        """
        Usage:
    ...
    async def cmd_oynat(self, player, channel, author, permissions, leftover_args, song_url):
        """
        Nasıl Yapılır?:
            {command_prefix}play Link Olarak Ata Bilirsiniz!
            {command_prefix}play Yazı olarak Yaza Bilirsiniz!
        """
        ...
        async with self.aiolocks[_func_() + ':' + author.id]:
            ...
            if 'entries' in info:
                ...
                reply_text = " **%s** Adlı Şarkı Sıraya Koyuldu Lütfen 10 Saniye Bekleyiniz. %s"
                btext = entry.title
    ...
    async def cmd_joinserver(self, channel, server, author, voice_channel):
        """
        Usage:
    ...
    async def cmd_durdur(self, player):
        """
        Usage:
    ...
    async def cmd_Devam(self, player):
        """
        Usage:
            {command_prefix}Devam
    ...
    async def cmd_atla(self, player, channel, author, message, permissions, voice_channel):
        """
        Atlar Direkt
            {command_prefix}atla
    ...
    async def cmd_ses(self, message, player, new_volume=None):
        """
        Usage:
        ...
        if not new_volume:
            return Response('Geçerli Ses: `%s%%`' % int(player.volume * 100), reply=True, delete_after=20)
        ...
        try:
            ...
            raise exceptions.CommandError('{} Naptın Kardeş Okadar Ses Açamassın'.format(new_volume), expire_in=20)
        ...
        if 0 < new_volume <= 100:
            ...
            return Response('Ses Yenilendi. %d Ten %d' % (old_volume, new_volume), reply=True, delete_after=20)
    ...
    async def cmd_q(self, channel, player):
        ...
        if not lines:
            lines.append(
                'Burada Herhangi Bir Şarkı Bulunmamaktadır! Şarkı Eklemek İçin {}oynat.'.format(self.config.command_prefix))

        message = '\n'.join(lines)
        return Response(message, delete_after=30)

    async def cmd_temizle(self, message, channel, server, author, search_range=50):
        """
        Usage:
        ...
        if self.user.bot:
            if channel.permissions_for(server.me).manage_messages:
                ...
                return Response('Sohbet Temizlendi {} :flag_tr: {}.'.format(len(deleted), '' * bool(deleted)), delete_after=15)
        ...
        return Response('mesaj temizlendi {} {}.'.format(deleted, '' * bool(deleted)), delete_after=6)
